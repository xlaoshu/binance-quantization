...
class Message:
    ...
    def sell_limit_msg(self, market, quantity, rate, profit_usdt=0):
        ...
        try:
            ...
            if res['orderId']:
                buy_info = "报警：币种为：{cointype}。卖单价为：{price}。卖单量为：{num}.盈利usdt数为:{profit_usdt}".format(cointype=market,
                                                                                                     price=rate,
                                                                                                     num=quantity,
    ...
    def sell_limit_future_msg(self, market, quantity, price):
        ...
        try:
            ...
            if res['orderId']:
                buy_info = "报警：币种为：{cointype}。卖出做空价格为：{price}。数量为：{num}".format(cointype=market, price=price,
                                                                                num=quantity)
                self.dingding_warn(buy_info)
                return res
        except BaseException as e:
    ...
    def buy_limit_future_msg(self, market, quantity, price, profit_usdt=None):
        ...
        try:
            ...
            if res['orderId']:
                buy_info = "报警：币种为：{cointype}。买入做多价格为：{price}。数量为：{num}。盈利USDT数为:{profit_usdt}".format(cointype=market,
                                                                                                       price=price,
                                                                                                       num=quantity,
                                                                                                       profit_usdt=profit_usdt)
                self.dingding_warn(buy_info)
