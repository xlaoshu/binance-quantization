...
class RunBetData:
    ...
    def get_spot_quantity(self, exchange_method=True):
        ...
        data_json = self._get_json_data()
        cur_step = data_json["runBet"]["spot_step"] if exchange_method else data_json["runBet"][
                                                                                "spot_step"] - 1  # 买入与卖出操作对应的仓位不同
    ...
    def get_future_quantity(self, exchange_method=True):
        ...
        data_json = self._get_json_data()
        cur_step = data_json["runBet"]["future_step"] if exchange_method else data_json["runBet"][
                                                                                  "future_step"] - 1  # 买入与卖出操作对应的仓位不同
